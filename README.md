...
The script prints a public verification URL:

```text
http://localhost:3000/verify/proof_...
```

And a proof report URL:
...
Tenant integration APIs expect an API key in either header:
...
API keys are hashed before being stored. Raw workflow inputs, event payloads, and outputs are hashed by default and are not persisted as plaintext unless a future connector explicitly chooses to store encrypted evidence.
...
The verification API returns a clear status:
...
Verification checks include proof existence, schema validity, proof package hash, workflow trace hash, event hashes, input/output/file hashes, storage metadata, and retrieval status.
...
In constrained environments without a package manager, the repository also includes a Node 24 fallback test command:
